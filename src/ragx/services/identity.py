...
async def register_tenant(
    ...
    if await session.scalar(select(Tenant).where(Tenant.name == tenant_name)):
...
async def list_api_keys(session: AsyncSession, *, ctx: TenantContext) -> list[ApiKey]:
    result = await session.scalars(
        select(ApiKey).where(ApiKey.tenant_id == ctx.tenant_id).order_by(ApiKey.created_at)
    )
    return list(result)
