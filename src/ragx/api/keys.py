...
async def create_key(
    body: ApiKeyCreateRequest, session: SessionDep, user: CurrentUserDep
) -> ApiKeyResponse:
    ctx = TenantContext(tenant_id=user.tenant_id, user_id=user.id)
    api_key, plain_text = await create_api_key(session, ctx=ctx, name=body.name)
...
async def list_keys(user: CurrentUserDep, session: SessionDep) -> list[ApiKeyResponse]:
    ctx = TenantContext(tenant_id=user.tenant_id, user_id=user.id)
    keys = await list_api_keys(session, ctx=ctx)
